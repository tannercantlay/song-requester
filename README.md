...
Neon. Fastify serves both the API and the built SPA from one origin. $0/mo,
about 20 minutes, most of it waiting on the first Docker build.

**Full runbook: [DEPLOY.md](DEPLOY.md)** — Neon setup, secrets, the Render
Blueprint flow, creating the first admin, Spotify, and the keep-alive pinger.

Four things worth knowing going in, because they're the ones that catch people:

- **Use New → Blueprint in Render, not New → Web Service.** Only a Blueprint
  instance reads `render.yaml`; a hand-created Web Service ignores it entirely
  and you lose every setting it pins.
- **`WEB_ORIGIN` needs a URL that doesn't exist yet.** Render assigns the
  hostname when the service is created, so set a placeholder, then come back and
  fill in the real one. It's stable from then on, and `sync: false` means
  redeploying can't overwrite it.
- **You start with an empty database.** The container applies migrations on boot
  but seeds nothing, so the catalog and the first admin are yours to create.
  They live in Neon, so they survive every restart and redeploy afterwards.
- **Spotify is optional.** Leave those three variables blank and upload a
  spreadsheet at `/admin/catalog` instead — it's a first-class path, not a
  fallback.
...
platform and why `render.yaml` pins `numInstances: 1`. Free instances can't
scale anyway; it becomes yours to enforce the moment you upgrade.
